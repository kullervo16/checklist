(function() {
    ...
    app.controller('checklistController', function($scope,$http,$window,$location,$anchorScroll,$interval) {
        ...
        function addErrorAction(step, error) {
            $http.put('rest/checklist/'+$location.search().id+"/"+step.id+'/error', error)
                .success(function (data,status,headers,config) {
                    $scope.data = data;   
                    reposition();
                }).error(function (data,status,headers,config) {
        ...
        function addTextAnswer(step, answer) {
            $http.put('rest/checklists/'+$location.search().id+"/"+step.id+'/answer', answer)
                .success(function (data,status,headers,config) {
                    $scope.data = data;   
                    reposition();
                }).error(function (data,status,headers,config) {
        ...
        function launchSubChecklist(step) {
            $http.put('rest/templates/'+step.subChecklist+'/createChecklist?'+"&parent="+$location.search().id)
                .success(function (data,status,headers,config) {
                    $window.location.href = './checklist.html?id='+data;                      
                }).error(function (data,status,headers,config) {
                    console.log('Error creating new checklist');
